...
map.getView().fit([-7935476.331409, -3501852.558861, -7927688.518141, -3496400.799971], map.getSize());
...
function projectPopup(feature, layer) {
    var iniciativa = feature.get('INICIATIVA');
    var categoria = feature.get('categoria');
    var fuente = feature.get('fuente_fin') || feature.get('FINANCIAMI');
    var estado = feature.get('estado_simple') || feature.get('ESTADO');
    var tematica = feature.get('TEMATICA');
    var ano = feature.get('ANO');
    var externo = parseMoney(feature.get('EXTERNO'));
    var municipal = parseMoney(feature.get('MUNICIPAL'));
    var monto = parseMoney(feature.get('MONTO'));
    if (!monto && (externo || municipal)) monto = externo + municipal;
    var badges = [];
    if (categoria) badges.push('<span class="popup-badge">' + escapeHtml(categoria) + '</span>');
    if (tematica) badges.push('<span class="popup-badge">' + escapeHtml(tematica) + '</span>');
    return '<div class="popup-card">' +
        '<h3>' + escapeHtml(iniciativa || layer.get('popuplayertitle')) + '</h3>' +
        '<div class="popup-badges">' + badges.join('') + '</div>' +
        '<dl class="popup-grid">' +
        '<dt>Estado</dt><dd>' + escapeHtml(estado) + '</dd>' +
        '<dt>Fuente</dt><dd>' + escapeHtml(fuente) + '</dd>' +
        '<dt>Año</dt><dd>' + escapeHtml(ano) + '</dd>' +
        '</dl>' +
        '<div class="popup-finance" aria-label="Detalle de financiamiento">' +
            '<div class="popup-finance-title">Detalle de financiamiento</div>' +
            '<div class="popup-finance-grid">' +
                '<div class="popup-finance-card"><span>Externo</span><strong>' + escapeHtml(formatMonto(externo)) + '</strong></div>' +
                '<div class="popup-finance-card"><span>Municipal</span><strong>' + escapeHtml(formatMonto(municipal)) + '</strong></div>' +
                '<div class="popup-finance-card popup-finance-total"><span>Total</span><strong>' + escapeHtml(formatMonto(monto)) + '</strong></div>' +
            '</div>' +
        '</div>' +
        '<div class="popup-small">Capa: ' + escapeHtml(layer.get('popuplayertitle')) + '</div>' +
        '</div>';
}

function buildFeaturePopup(feature, layer, currentFeatureKeys) {
    if (feature.get('INICIATIVA')) {
        return projectPopup(feature, layer);
    }
    return '<table>' + createPopupField(feature, currentFeatureKeys, layer) + '</table>';
}
        ...
        if (typeof clusteredFeatures !== "undefined") {
            if (doPopup) {
                for(var n=0; n<clusteredFeatures.length; n++) {
                    ...
                    popupText += '<li>' + buildFeaturePopup(currentFeature, layer, currentFeatureKeys) + '</li>';    
                }
            }
        } else {
            currentFeatureKeys = currentFeature.getKeys();
...
// Keep the layer panel visible on touch and small screens.
// The custom right-side panel already manages its own collapse state,
// so we do not auto-hide the OpenLayers layer switcher panel here.
